...
const index = async (req, res) => {
  try {
    ...
    if (req.query.search) search.fullname = { $regex: `^${req.query.search}`, $options: "i" };
    ...
    if (!resultUsers) throw { code: 404, message: "USERS_NOT_FOUND" };
...
const details = async (req, res) => {
  try {
    ...
    if (!resultUser) throw { code: 404, message: "USER_NOT_FOUND" };
